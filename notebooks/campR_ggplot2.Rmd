...
# We can change the breaks of the histogram in this way
  (p1 = p0 + geom_histogram(breaks=seq(1900,2010,by=1)))
...
# Change the facet labels from {0,1} to self-descriptive labels
# That's not how it works:
  movies$Action = factor(movies$Action, c(0,1), c('No action', 'Action'))
  p2 + facet_grid(factor(Animation) ~ Action)
```
...
# or changing various properties of the theme
  p2 + theme(axis.ticks = element_line(size = 2),
             panel.background = element_rect(fill="#AAE9AA"))
  
# it handles also the legend 
